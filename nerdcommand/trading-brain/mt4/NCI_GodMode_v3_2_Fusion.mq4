...
#property description "NCI GodMode v3.2 Fusion - multi-port entry engine with unified dynamic stop/pip catcher."
#property strict
...
extern double InpM5TakeProfitRR     = 0.45;
...
extern double InpZoneRR             = 0.5;     // inverted reward/risk from the high-WR file
...
extern double InpQuickBEAtR         = 0.25;    // ScalpBot BE trigger
...
void TryOpenByPort(int direction,int port)
{
   ...
   string cmt="NCI v3.2 "+PortName(port);
...
void ManageUnifiedStops()
{
   ...
   for(int i=OrdersTotal()-1;i>=0;i--)
   {
      ...
      // USD-trigger secure fixed pip trail from GodMode v3.1.
      if(InpUseSecureTrail && SpreadPips()<=InpMaxSpreadPips)
      {
         double pnl=OrderProfit()+OrderSwap()+OrderCommission();
         if(pnl>=InpSecureProfitUSD)
         {
            double tr=InpSecureTrailPips*PipPoint;
...
bool TryEntryPorts(int buy,int sell)
{
   ...
   // Port 1: original GodMode main path (kept intact).
   if(InpUseGodModePort && SpreadPips()<=InpMaxSpreadPips && ABCAllowsEntry())
   {
      if(buy>=InpMinConfluence && buy>sell)
      {
         if(HTFGateAllowBuy()){ if(InpTradingEnabled) TryOpenByPort(1,PORT_GODMODE); else Print("Fusion REPORT would BUY GodMode score=",buy); return(true); }
      }
      if(sell>=InpMinConfluence && sell>buy)
      {
         if(HTFGateAllowSell()){ if(InpTradingEnabled) TryOpenByPort(-1,PORT_GODMODE); else Print("Fusion REPORT would SELL GodMode score=",sell); return(true); }
      }
   }
   ...
   if(scalpSig!=0)
   {
      if(InpTradingEnabled) TryOpenByPort(scalpSig,PORT_HYBRIDSCALP); else Print("Fusion REPORT would ",(scalpSig>0?"BUY":"SELL")," HybridScalp");
      return(true);
   }
   ...
   if(m5Sig!=0)
   {
      if(InpTradingEnabled) TryOpenByPort(m5Sig,PORT_M5ARROW); else Print("Fusion REPORT would ",(m5Sig>0?"BUY":"SELL")," M5Arrow");
      return(true);
   }
   ...
   if(neuroSig!=0 && neuroPort!=PORT_NONE)
   {
      if(InpTradingEnabled) TryOpenByPort(neuroSig,neuroPort); else Print("Fusion REPORT would ",(neuroSig>0?"BUY":"SELL")," ",PortName(neuroPort));
      return(true);
   }
   ...
   if(zoneSig!=0)
   {
      if(InpTradingEnabled) TryOpenByPort(zoneSig,PORT_ZONE); else Print("Fusion REPORT would ",(zoneSig>0?"BUY":"SELL")," ZoneHighWR");
      return(true);
   }
...
int OnInit()
{
   ...
   Print("===== NCI GodMode v3.2 FUSION - AUTONOMOUS EA =====");
   ...
   Print("v3.2 FUSION PORTS: main=",InpUseGodModePort," scalp=",InpUseHybridScalpPort," m5=",InpUseM5ArrowPort," neuro=",InpUseNeuroPorts," zone=",InpUseZoneEntryPort," altABCBlock=",InpABCBlocksAltPorts);
   Print("v3.2 UNIFIED STOP: ",InpUseUnifiedStopManager," secure=",InpUseSecureTrail," quickR=",InpQuickBEAtR," trailR=",InpTrailAtR," hardLock=",InpHardLockPips,"p | STACK max=",EffectiveMaxOpenTrades());
...
void OnTick()
{
   ...
   if(InpLogConfluence)
      Print("Fusion scores BUY=",buy,"/15 SELL=",sell,"/15 stage=",StageLabel(DetectABCStage(0,1))," lastPort=",g_lastEntryPort);
